...
if (!empty($package_product_ids)) { ?>
	...
	<div class="bd_items_div bd_package_items_div i_clearfix theme_color_<?= self::$package_theme_color ?> bd_select_wrap row" id="bd_checkout">
			...
			<div class="bd_items_div_inner_row row">
				...
				foreach ($package_product_ids as $opt_i => $prod) {
					...
					if ($prod['type'] == 'free' || $prod['type'] == 'off') { ?>
						...
						<div class="col col-lg-4 col-md-4 col-sm-12 col-xs-12 item-selection col-hover-focus bd_item_div bd_item_div_<?php echo trim($prod['bun_id']) ?> bd_c_package_option <?= (self::$package_default_id == $prod['bun_id']) ? 'bd_selected_default_opt bd_active_product' : '' ?>" data-type="<?php echo trim($prod['type']) ?>" data-bundle_id="<?php echo trim($prod['bun_id']) ?>" data-coupon="<?= round((float)$i_coupon, 0) ?>">
						<?php
					} else { ?>

							<li class="col col-lg-4 col-md-4 col-sm-12 col-xs-12 item-selection col-hover-focus bd_item_div bd_item_div_<?php echo trim($prod['bun_id']) ?> bd_c_package_option <?= (self::$package_default_id == $prod['bun_id']) ? 'bd_selected_default_opt bd_active_product' : '' ?>" data-type="<?php echo trim($prod['type']) ?>" data-bundle_id="<?php echo trim($prod['bun_id']) ?>" data-coupon="<?= round((float)$i_coupon, 0) ?>">
							<?php
						} ?>
									...
									<div class="bd_c_package_content">
										...
										<div class="bd_c_package_info text-center">
											...
											?>
												...
												<div class="pi-price-total">
													<span><?php echo wc_price($i_price * $i_total_qty); ?></span>
												</div>
	...
	add_action('wp_footer', function () { ?>
		...
		<script>
			...
			/**
			 * Show/hide variations containers
			 */
			$('.bd_item_div').each(function(index, container) {
				...
				$(container).mousedown(function(e) {
					...
					$(container).addClass('bd_active_product');
					$(container).find('.col-inner').addClass('bd_active');

					$('.bd_product_variations').hide();
					$('#bd_product_variations_' + $(this).data('bundle_id')).show();
