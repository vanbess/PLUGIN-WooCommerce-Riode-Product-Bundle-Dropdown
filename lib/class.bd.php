...
class BD
{
    ...
    private static function init_hooks()
    {
        ...
        add_action('wp_enqueue_scripts', array('BD', 'load_resources'));

        // action ajax add products to cart
        add_action('wp_ajax_bd_add_to_cart_multiple', array('BD', 'bd_add_to_cart_multiple'));
        add_action('wp_ajax_nopriv_bd_add_to_cart_multiple', array('BD', 'bd_add_to_cart_multiple'));

        // action get price summary table
        add_action('wp_ajax_bd_get_price_summary_table', array('BD', 'bd_get_price_summary_table'));
        add_action('wp_ajax_nopriv_bd_get_price_summary_table', array('BD', 'bd_get_price_summary_table'));

        // action get price variation product bd
        add_action('wp_ajax_bd_get_price_variation_product', array('BD', 'bd_get_price_variation_product'));
        add_action('wp_ajax_nopriv_bd_get_price_variation_product', array('BD', 'bd_get_price_variation_product'));

        // action get price bd package
        add_action('wp_ajax_bd_get_price_package', array('BD', 'bd_get_price_package'));
        add_action('wp_ajax_nopriv_bd_get_price_package', array('BD', 'bd_get_price_package'));

        add_action('woocommerce_update_cart_action_cart_updated', array('BD', 'on_action_cart_updated'), 20, 1);
        add_action('woocommerce_cart_calculate_fees', array('BD', 'add_calculate_bundle_fee'), PHP_INT_MAX);
    ...
    public static function add_calculate_bundle_fee($cart)
    {
        ...
        elseif ($bundle_selection['selValue'] == 'off') {
            if ($cart_qty >= $bundle_selection['selValue_off']['quantity']) {
                ...
                if ($discount > 0) {
                    $disc_name = sprintf(__('Buy %s + Get %d&#37; Off', 'bd'), $bundle_selection['selValue_off']['quantity'], $bundle_selection['selValue_off']['cupon']);
                    $cart->add_fee($disc_name, -$discount, true);
                }
            }
        }
    ...
    public static function on_action_cart_updated($cart_updated)
    {
        if ($cart_updated) {
            ...
            if (isset($bundle_selection)) {
                ...
                remove_action('woocommerce_cart_calculate_fees', array('BD', 'add_calculate_bundle_fee'), PHP_INT_MAX);
            }
        }
    }
